import { articles, categories } from "@/lib/strapiClient";
...
export const Route = createFileRoute("/demo/strapi")({
  ...
  loader: async () => {
    const { data: strapiArticles } = await articles.find({
      // This tells Strapi to include the 'cover' relation in the response
      populate: ["author"],
    });
    const { data: strapiCategories } = await categories.find({
      // This tells Strapi to include the 'cover' relation in the response
    });
    return {
...
function RouteComponent() {
  ...
  const strapiCategories = Route.useLoaderData();
  // console.log("strapiCategories", strapiCategories.categories);
  return (
